...
## 2026-09-05 10:43 — run closed early, by request

Steven redirected to the AI cluster before #547 was started. Closing out
rather than rewriting this plan: #544 and #543 are complete and closed on
GitHub, and #547 is untouched — not half-done — so it goes back on the
open list exactly as it was.

`python tools/run_tests.py` — **90 of 90 test files passed** (85 at the
start of this run). Five new test files: test_change, test_change_api,
test_change_ui, test_compliance, test_compliance_ui.
